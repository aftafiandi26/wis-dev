...
<div class="row">
   <div class="col-lg-12">
            <h1 class="page-header">Leave Summary for All Employes <sup>({{ $start . " - " . $end }})</sup></h1>           
    </div>  
</div>
<div class="row">
    <div class="col-lg-12">
        <a class="btn-sm btn btn-default" id="search" href="{{ route('leave/summary/employes/index') }}">back</a>
    </div>
</div>
            ...
            <div class="form-group">
                ...
                <select name="select" id="date" class="form-control" required>
                    ...
                    @foreach ($department as $depts)
                        <option value="{{ $depts->dept_category_name }}" @if ($depts->dept_category_name === $dept)
                            selected
                        @endif>{{ $depts->dept_category_name }}</option> 
                    @endforeach
                </select>
                <input type="date" name="startDate" id="date" class="form-control" required value="{{ $start }}">
                <input type="date" name="endDate" id="date" class="form-control" required value="{{ $end }}">
            </div>
...
$('#tables').DataTable({
    ...
    ajax: "{{ route('leave/summary/employes/find/data', [$start, $end, $dept]) }}",
    columns: [
