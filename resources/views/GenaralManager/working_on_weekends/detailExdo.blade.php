...
<div class='modal-header'>
    ...
    <h3 class='modal-title text-center' id='showModalLabel2'>Weekend Crew <span id="homan">(Exdo)</span> on <i>{{ date('Y-m-d', strtotime($works[0]['start'])) }}</i>.</h3>
</div>
...
<div class='modal-footer'>
    @if ($getId->ap_producer == true)
        <a href={{ route('gm/working-on-weekends/approved/exdo', $getId->id) }} class='btn btn-sm btn-default' id="approved">All Approved [<span id="getApp">{{ $allApproved }}</span>]</a>
        <a href="{{ route('gm/working-on-weekends/disapproved/exdo', $getId->id) }}" class='btn btn-sm btn-default' id="disapproved">All Disapproved [ <span id="asd">{{ $allDisapproved }}</span> ]</a>        
    @else
