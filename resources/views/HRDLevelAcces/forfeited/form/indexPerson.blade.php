            ...
            <div class="form-group">
                ...
                <select name="emp" id="emp" class="form-control">
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @if ($user->id == $id)
                            selected
                        @endif>{{ $user->getFullName() }}</option>
                    @endforeach
                </select>
                <button type="submit" class="btn btn-sm btn-default" id="findButton">find</button>
                <a href="{{ route('forfeited/form/index') }}" class="btn btn-sm btn-default" id="findButton">back</a>
            </div>
        </div>        
    </div>
</form>
...
$('#forfaitedTables').DataTable({
    ...
    responsive: true,
    ajax: {
        url: '{{ route('forfeited/indexPerson/datatablesPerson', $id) }}',
        type: 'GET',     
    },
    columns: [
