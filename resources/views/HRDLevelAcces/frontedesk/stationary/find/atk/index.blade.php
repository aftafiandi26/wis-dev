...
<div class="row">
<div class="col-lg-12">   
    <a href="{{ route('stationery/atk/month/index', date('Y-m', strtotime('previous month', strtotime($month))))}}" class="btn btn-sm btn-default previous"><i class="fa fa-arrow-left"></i> {{ $previous }}</a>
    <a href="{{ route('stationery/atk/month/index', date('Y-m', strtotime('next month', strtotime($month))))}}" class="btn btn-sm btn-default previous">{{ $next }} <i class="fa fa-arrow-right"></i></a>
    <a href="{{route('stationery/atk/stocked/add')}}" class="btn btn-sm btn-default stock pull-right"> <i class="fa fa-plus"></i> Add Stock</a>
    <br>
    <br>
    <label style="margin-bottom: -10px;">Periode: {{date('F Y', strtotime($month))}}</label><br>
    ...
    <table id="myTable" class="table table-hover table-bordered table-condensed table-striped">
        <thead>
            <tr>
                ...
                <th colspan="{{ $lastDay }}" style="text-align: center;">Date Items Out Transaction <sup><i>{{date('F Y', strtotime($month))}}</i></sup></th>
                    ...
                    <tr class="waters">
                        ...
                        <td style="text-align: center;">
                            <a data-toggle="modal" data-target="#showModal" title="view" data-role="{{ route('stationery/atk/modal', [$value->kode_barang, $i]) }} " class="ahover">
                                {{ Stationary_transaction::where('key_param', $key_param)->where('kode_barang', $value->kode_barang)->where('status_transaction', 2)->whereDate('date_out_stock', date('Y-m', strtotime($month)).'-'.$i)->pluck('out_stock')->sum() }} 
                            </a>                                              
                        </td>
                        @endfor              
                        <td style="text-align: center;">
                        ...
                        <td style="text-align: center;">
                            <span id="in_purchase">
                                {{ Stationary_transaction::where('kode_barang', $value->kode_barang)->where('status_transaction', 1)->whereYear('date_in_stock', date('Y', strtotime($month)))->whereMonth('date_in_stock', date('m', strtotime($month)))->pluck('in_stock')->sum() }}
                            </span>
                        </td>
                        <td style="text-align: center;">
                        ...
                        <td><a href="{{route('stationery/atk/month/purchase/add', [$value->id, $month])}}" class="btn btn-xs btn-default" id="in">In</a></td>
                        <td><a href="{{route('stationery/atk/month/out/index', [$value->id, $month])}}" class="btn btn-xs btn-default @if ($getStocked <= 0)
                            hidden
