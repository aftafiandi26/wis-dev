<form action="{{ route('hr/summary/attendance/summary/employes/delete/post', $data->id) }}" method="post" >
    {{ csrf_field() }}
    <div class='modal-header'>
        <button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>
        <h4 class='modal-title text-center' id='showModalLabel'>Attendance</h4>
    </div>
    <div class='modal-body'>
        <div class="row">
            <div class="col-lg-12"> 
                <input type="hidden" name="selectEmp" value="{{ $dataArray[0] }}" readonly>
                <input type="hidden" name="empDateStarted" value="{{ $dataArray[1] }}" readonly>
                <input type="hidden" name="empDateEnded" value="{{ $dataArray[2] }}" readonly>   
