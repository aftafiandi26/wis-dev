...
@section('body')
<div class="row">
    <div class="col-lg-12"><h1 class="page-header">History - Working on Weekends {{ request()->input('employee', null) }}</h1>
    </div>
</div>
        ...
        <form action="{{ route('hrd/weekend-crew/history/detail/emp') }}" method="post" class="form-inline">
            ...
            <select name="employee" id="employee" class="form-control" required>
                ...
                @foreach ($prods as $prod)  
                    <option value="{{ $prod->id }}" @if (request()->input('employee', null) == $prod->id)
                        selected
...
$('table#tables').DataTable({
    ...
    ajax: {
        url: '{{ route('hrd/weekend-crew/history/detail/emp/data') }}',
        type: 'POST',
        data: { 
            employee: function () { return $('#employee').val(); },
