...
@section('body')
 ...
 <div class="row">
  ...
  <div class="row">
    <form class="form-horizontal" action="{{route('postInputDataOfficerMobileMap', [$animasi->id])}}" method="POST" enctype="multipart/form-data">     
    ...
    <div class="form-group">
      ...
      <div class=" col-lg-8">      
        <select class="form-control" id="workstation" name="workstation">
          ...
          <optgroup label="Move to Laptop">
            <option value=""> </option>
            <?php foreach ($laptop_availability as $laptop): ?>
              <option value="{{$laptop->hostname_laptop}}">{{$laptop->hostname_laptop}}</option>
            <?php endforeach ?>
          </optgroup>
     ...
     <div class="form-group">
      <label class="control-label  col-lg-1" for="secondary_workstation">Secondary Workstation:</label>
      <div class=" col-lg-8">      
        <select class="form-control" id="secondary_workstation" name="secondary_workstation">      
          <optgroup label="Current Workstation">
            <option value="{{$animasi->secondary_workstation}}">{{$animasi->secondary_workstation}}</option>
          </optgroup>      
            <option value=""> </option>
            <?php foreach ($laptop_availability as $laptop): ?>
              <option value="{{$laptop->hostname_laptop}}">{{$laptop->hostname_laptop}}</option>
    ...
    <div class="form-group">
     ...
     <div class="form-group">        
      <div class=" col-lg-offset-1  col-lg-12">       
          ...
          <a href="{{route('indexMAPOfficer')}}" class="btn btn-sm btn-warning" title="Back to Index MAP">Back</a>
      </div>     
    </div>
