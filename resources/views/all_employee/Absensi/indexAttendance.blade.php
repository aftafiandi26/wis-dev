...
<style>
    ...
    a#checkOut:hover, a#checkIn:hover {
    ...
    .loader {
        ...
        border: 8px solid lightgray; /* Warna garis loading */
        border-top: 8px solid transparent; /* Membuat bentuk cincin */
    ...
    <div class="col-lg-4">
        <table class="table-striped table table-condensed table-borderless table-hover" id="tables">
            <thead>
                ...
                <tr>
                    <th>Datetime</th>
                    <th>: {{ $date->toFormattedDateString() }} <span id="jam"></span>:<span id="menit"></span>:<span id="detik"></span></th>
                </tr>
                <tr>
                    <th>Work From</th>
                    <th>: <select name="work_from" id="work_from" required>
                            ...
                            @if ($attendance)
                                <option value="wfs" @if ($attendance->status_in === "wfs")
                                selected @else disabled style="color: whitesmoke;" @endif>WFS</option>
                                <option value="wfh" @if ($attendance->status_in === "wfh")
                                selected @else disabled style="color: whitesmoke;" @endif>WFH</option>
                            @else
                    ...
                    <th colspan="2" class="text-right">
                        @if ($attendance)
                            @if ($attendance->out == false)
                                <a  class="btn btn-sm btn-default" id="checkOut" data-toggle="modal" data-target="#showModal" data-role="{{ route('attendance/checkOut') }}">Check Out</a>  
                            @else 
                                <span class="thanks">Thank you for filling out your attendance!!</span>                                                          
                            @endif
                        @else
                            <a class="btn btn-sm btn-default" id="checkIn" data-toggle="modal" data-target="#showModal" data-role="{{ route('attendance/checkin') }}">Check In</a>
                        @endif  
                    </th>
                </tr>
            </thead>
