<div class="row">
    <div class="col-lg-12">
        Dear
        @if ($formOvertime->app_coor === 0)
            {{ $formOvertime->coordinator->getFullName() }}
        @endif        
        @if ($formOvertime->app_coor === 1)
            Sir/Madam
        @endif,
        <br><br>
        There is a form remote access request by: <b>{{ $formOvertime->user->getFullName() }}</b> <i>({{ $formOvertime->user->position }})</i> project <b>{{ $formOvertime->user->getProjectName($formOvertime->user->project_category_id_1) }}</b> that requires verify.<br><br>
        <table border="1">
            <tr>
                <th>Request By</th>
                <td>:</td>
