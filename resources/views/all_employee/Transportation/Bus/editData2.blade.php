...
@endif
<form class="form-horizontal" action="{{route('editData2RescheduleDataBooking', [$getData->id, $getData->key_transportation])}}" method="post">  
  {{ csrf_field() }}
  <div class="row">  
  <!--  -->
  ...
  <div class="form-group" onclick="departuree()">
      <label class="control-label col-sm-2" for="departure">Arrival Time:</label>
      <div class="col-sm-5">
...
<script>
function departuree() {
  document.getElementById("departuree").innerHTML = '<label class="control-label col-sm-2" for="departure_destination">Destination:</label><div class="col-sm-5"><input type="text" class="form-control" id="departure_destination" name="departure_destination" value="{{$getData->destination}}" required="true" readonly="true"></div>';
}
</script>
 @stop 
